...
const getCourseCurriculum = async (
  userId,
  courseId
) => {
  ...
  const lectures =
    await CourseLecture.find({
      ...
      .select(
        "_id title duration order sectionId isPreview"
      )
      .sort({
        order: 1,
      })
  ...
  const completed =
    new Set(
      enrollment.completedLectures.map(
        id => id.toString()
      )
    );
