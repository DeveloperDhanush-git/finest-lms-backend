...
const getLecturesBySection = async (req, res, next) => {
  try {
...
const uploadLectureResource = async (req, res, next) => {
  try {
    if (!req.file) {
...
const removeLectureVideo = async (req, res, next) => {
  try {
    ...
    return res.status(200).json({
      ...
      message: 'Lecture video removed successfully.',
    });
  } catch (error) {
    next(error);
  }
...
const getLectureVideoStatus = async (req, res, next) => {
  try {
    const status = await lectureService.getLectureVideoStatus(
      req.params.id,
